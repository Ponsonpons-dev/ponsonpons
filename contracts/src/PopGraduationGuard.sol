...
/**
 * @title PopGraduationGuard
 * @notice Stateless preflight for a graduation's Uniswap V4 seed. It keeps
 * the tick and liquidity math outside PopLaunchFactory's runtime bytecode
 ...
 *
 * The preflight has to mirror the whole downstream call graph rather than
 * the PositionManager's ABI field widths alone. Phase one is irreversible:
 * it marks the curve graduated and moves its reserves to the factory, so a
 * seed that passes here and reverts in V4 leaves the launch permanently
 * unseedable and recoverable only through the delayed, fixed-recipient
 * rescue path. Adapted from the verified PonsV2GraduationGuard.
 */
contract PopGraduationGuard {
    int24 private constant MIN_USABLE_TICK = -887272;
    int24 private constant MAX_USABLE_TICK = 887272;

    /**
     * @dev V4 carries pool balance changes in a `BalanceDelta` whose halves
     * are `int128`, and `Pool.modifyLiquidity` narrows each side with
     ...
     * @param quoteToken Quote asset of the pool.
    ...
    function assertSeedable(
        ...
        // Two ERC-20s sort by address. The seed price is
        // orientation-dependent, so the ordering here must match the PoolKey
        // the factory will build.
        bool quoteIsCurrency0 = quoteToken < token;
    ...
    /**
     * @notice Verifies a seed of these proportions mints under either
     * currency ordering.
     ...
     * range is symmetric about 1 and the liquidity formula is invariant
     * under inverting the price and swapping the amounts with it.
     */
    ...
    /**
     * @dev Models the price and liquidity rejections of the real mint for
     * one currency ordering. Amount bounds are the caller's to enforce.
     */
    function _assertSeedable(int24 tickSpacing, uint256 amount0, uint256 amount1) private pure {
        ...
        // liquidity is the entire `liquidityGross` at each of them. V4
        // reverts with TickLiquidityOverflow once a tick's gross liquidity
        // passes the cap its spacing implies, which is an independent
