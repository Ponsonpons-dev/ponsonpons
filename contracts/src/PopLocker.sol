...
import {IERC721ReceiverLike} from "./interfaces/IPop.sol";

/**
 * @title PopLocker
 * @notice Permanently holds the graduated Uniswap V4 position NFT for every
 * $POP launch, plus the virtual-reserve token remainder that cannot enter
 * the graduated pool without lowering its opening price. There is no
 * `collectFees()` here: fee collection and distribution belong entirely to
 * PopHook and PopFeeEscrow, since a V4 position accrues fees inside the
 * singleton PoolManager rather than on the NFT itself.
 *
 * This contract exposes no withdrawal or arbitrary-call function, so locked
 * liquidity can never be removed by anyone, the protocol owner included.
 * Ownership exists only to perform the one-time factory wiring.
 * Adapted from the verified PonsV2LaunchLocker.
 */
contract PopLocker is Ownable2Step, IERC721ReceiverLike {
